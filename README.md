...
```ini
./uninstall [go] [flutter] [go-docker] [go-docker-hard] [flutter-docker-hard] [docker-hard]
```
...
| 👨‍⚕️ **Доктор**            | `http://localhost:{FRONTEND_PORT+2}`                 | Окно доктора              |
| 🖥️ **Электронное табло** | `http://localhost:{FRONTEND_PORT+3}`                 | Электронное табло очереди |
